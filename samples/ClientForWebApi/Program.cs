...
{
    var books = await httpMediator.SendAsync<GetBooksQuery, Book[]>(new GetBooksQuery(), CancellationToken.None);
...
async ValueTask CreateNewBook()
{
    ...
    var book = await httpMediator.SendAsync<CreateBookCommand, Book>(createBookCommand, CancellationToken.None);
    Console.WriteLine("New book info:");
    Console.WriteLine($"id:{book.BookId}, title: {book.Title}");
    Console.WriteLine();
}
